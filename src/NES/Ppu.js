...
class Ppu {
  ...
  loadRegister(address) {
    ...
    switch (address) {
      ...
      case 0x2007: {
        if (
          (this.currentVRamAddress & 0x3fff) >= 0 &&
  ...
  load(address) {
    ...
    if (address >= 0x2000 && address < 0x3f00)
      return this.vRam.load(
        this.getNameTableAddressWithMirroring(address & 0x2fff)
      );

    if (address >= 0x3f00 && address < 0x4000) address = address & 0x3f1f;
  ...
  getNameTableAddressWithMirroring(address) {
    ...
    switch (this.rom.getMirroringType()) {
      ...
      case 1: // MIRRORINGS.HORIZONTAL:
        if (address >= 0x2000 && address < 0x2400) baseAddress = 0x2000;
        else if (address >= 0x2400 && address < 0x2800) baseAddress = 0x2000;
        else if (address >= 0x2800 && address < 0x2c00) baseAddress = 0x2400;
        else baseAddress = 0x2400;

        break;

      case 2: // MIRRORINGS.VERTICAL:
        if (address >= 0x2000 && address < 0x2400) baseAddress = 0x2000;
        else if (address >= 0x2400 && address < 0x2800) baseAddress = 0x2400;
        else if (address >= 0x2800 && address < 0x2c00) baseAddress = 0x2000;
        else baseAddress = 0x2400;
      ...
      case 3: // MIRRORINGS.FOUR_SCREEN:
        ...
        else if (address >= 0x2800 && address < 0x2c00) baseAddress = 0x2800;
        else baseAddress = 0x2c00;
  ...
  renderPixel() {
    ...
    if (backgroundVisible === true && spritesVisible === true) {
      if (spritePixel === -1) {
        ...
        if (backgroundPixel === c) c = spritePixel;
        else c = spritePriority === 0 ? spritePixel : backgroundPixel;
      }
    } else if (backgroundVisible === true && spritesVisible === false) {
      c = backgroundPixel;
    } else if (backgroundVisible === false && spritesVisible === true) {
      if (spritePixel !== -1) c = spritePixel;
    }

    // TODO: fix me

    if (this.ppumask.emphasisRed() === true) c = c | 0x00ff0000;
    if (this.ppumask.emphasisGreen() === true) c = c | 0x0000ff00;
    if (this.ppumask.emphasisBlue() === true) c = c | 0x000000ff;
  ...
  getBackgroundPixel() {
    ...
    let lsb =
      (this.patternTableHighRegister.loadBit(offset) << 1) |
      this.patternTableLowRegister.loadBit(offset);
    let msb =
      (this.attributeTableHighRegister.loadBit(offset) << 1) |
      this.attributeTableLowRegister.loadBit(offset);
    let index = (msb << 2) | lsb;

    // TODO: fix me

    if (this.ppumask.isGreyscale() === true) index = index & 0x30;
  ...
  fetch() {
    ...
    if ((this.cycle >= 257 && this.cycle <= 320) || this.cycle >= 337) return;
    ...
    if (this.cycle % 8 === 1) {
      ...
      this.attributeTableLowRegister.storeLowerByte(
        this.attributeTableLowLatch
      );
      this.attributeTableHighRegister.storeLowerByte(
        this.attributeTableHighLatch
      );
   ...
   */
  fetchNameTable() {
    this.nameTableLatch = this.load(
      0x2000 | (this.currentVRamAddress & 0x0fff)
    );
  }
  ...
  fetchPatternTableLow() {
    ...
    let index =
      this.ppuctrl.getBackgroundPatternTableNum() * 0x1000 +
      this.nameTableRegister.load() * 0x10 +
      fineY;
  ...
  countUpScrollCounters() {
    if (
      this.ppumask.isBackgroundVisible() === false &&
      this.ppumask.isSpritesVisible() === false
    )
      return;
